...
function Cart() {
  ...
  const checkOut = () => {
    if (cart.books.some((one) => one.book.quantity < one.quantity)) {
      setErrorList([
        "Please adjust item quantity to match with our in stock quantity before proceeding to checkout.",
      ]);
      return;
    }
      ...
      <div className="ui container grid white-bg">
        ...
        {cart.itemNum > 0 && (
          <div className="four wide column">
            <h5>
              Subtotal ({cart.itemNum} items): ${cart.subtotal}
            </h5>
            <button className="ui green button" onClick={checkOut}>
              CHECKOUT
            </button>
          </div>
        )}
